...
func _refresh_inventory() -> void:
	for slot in slots:
		...
		slot.texture = null
		slot.visible = false
		slot.scale = Vector2(1.0, 1.0)  # reset scale each refresh
	for i in range(min(GameProgress.inventory_items.size(), slots.size())):
		...
		if tex != null:
			...
			if item_name == "mushroom":
				slot.scale = Vector2(0.555, 0.555)
				slot.position = Vector2(500.0, -800.0)
			else:
				slot.scale = Vector2(1.0, 1.0)
				slot.position = Vector2(-37.0, -862.0)
...
func _get_texture_for_item(item_name: String) -> Texture2D:
	match item_name:
		...
		"mushroom":
			return mushroom  # THIS WAS MISSING
		_:
			return null
