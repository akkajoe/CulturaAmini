...
# New: cross-chain bridge settings
...
# Exact mushroom state while off-route
...
func _build_route_marker_path(from_route: TraversalRoute, to_route: TraversalRoute) -> Array[TraversalMarker]:
	...
	if from_route == null or to_route == null:
		return result
...
func _finalize_route_plan() -> void:
	...
	match after_route_plan_action:
		...
		AfterRoutePlanAction.START_JUMP_CHAIN:
			...
			var first_jump_pos: Vector2 = jump_chain_positions[0]
			target_offset = active_route.get_closest_offset_to_global(first_jump_pos)
...
func _update_ground(delta: float) -> void:
	...
	if absf(diff) < 2.0:
		...
		if jump_chain_active and jump_chain_positions.size() > 0:
			var first_jump_target: Vector2 = jump_chain_positions.pop_front()
			_start_jump_to_position(first_jump_target)
			return
	...
	var max_off: float = active_route.ground_path.curve.get_baked_length()
	current_offset = clampf(current_offset, 0.0, max_off)
	...
	if jump_t >= 1.0:
		...
		if jump_chain_active:
			if jump_chain_positions.size() > 0:
				var next_pos: Vector2 = jump_chain_positions.pop_front()
				_start_jump_to_position(next_pos)
				return
			else:
				jump_chain_active = false
				...
				if jump_chain_needs_ground_jump and pending_target_route != null:
					...
					var final_route_point: Vector2 = pending_target_route.get_global_point_at_offset(pending_target_offset)
					_start_jump_to_position(final_route_point)
					return
...
func _update_ground_facing_and_tilt(dir_sign: float) -> void:
	...
	var a_local: Vector2 = curve.sample_baked(a_off)
	var b_local: Vector2 = curve.sample_baked(b_off)

	var a_global: Vector2 = active_route.ground_path.to_global(a_local)
	var b_global: Vector2 = active_route.ground_path.to_global(b_local)
	...
	var target_tilt: float = slope_tilt

	if dir.x < 0.0:
		target_tilt = -slope_tilt

	target_tilt *= 0.45
	target_tilt = clampf(target_tilt, deg_to_rad(-16.0), deg_to_rad(16.0))
...
func _update_hover_cursor() -> void:
	...
	var now_valid: bool = _is_hover_valid(mouse_pos)

	if now_valid == _hover_is_valid:
		return

	_hover_is_valid = now_valid
...
func _is_hover_valid(mouse_pos: Vector2) -> bool:
	...
	if _is_hovering_clickable_asset(mouse_pos):
		return false
...
func _get_same_chain_neighbors(point: TraversalJumpPoint) -> Array[TraversalJumpPoint]:
	...
	for chain_id: String in point.chain_ids:
		var prev_point: TraversalJumpPoint = point.get_prev_point_on_chain(chain_id)
		var next_point: TraversalJumpPoint = point.get_next_point_on_chain(chain_id)

		_add_unique_neighbor(result, prev_point)
		_add_unique_neighbor(result, next_point)
...
func _get_bridge_neighbors(point: TraversalJumpPoint) -> Array[TraversalJumpPoint]:
	...
	if not allow_cross_chain_bridge:
		return result
	...
	for other: TraversalJumpPoint in jump_points:
		...
		candidates.append({
			"point": other,
			"dist": d,
			"chain_key": _get_chain_key_for_point(other)
		})
...
func _get_graph_neighbors(point: TraversalJumpPoint) -> Array[TraversalJumpPoint]:
	...
	for p: TraversalJumpPoint in _get_same_chain_neighbors(point):
		_add_unique_neighbor(result, p)

	for p: TraversalJumpPoint in _get_bridge_neighbors(point):
		_add_unique_neighbor(result, p)
...
func _find_jump_graph_path(
	...
	while frontier.size() > 0:
		...
		neighbors.sort_custom(func(a: TraversalJumpPoint, b: TraversalJumpPoint) -> bool:
			var da: float = a.global_position.distance_to(target_point.global_position)
			var db: float = b.global_position.distance_to(target_point.global_position)
			return da < db
		)
...
func _build_jump_chain(start_point: TraversalJumpPoint, target_point: TraversalJumpPoint) -> Array[TraversalJumpPoint]:
	...
	if start_point.order < target_point.order:
		var cur_fwd: TraversalJumpPoint = start_point
		var guard_fwd: int = 0

		while cur_fwd != null and guard_fwd < 256:
			result.append(cur_fwd)
			if cur_fwd == target_point:
				return result
			cur_fwd = cur_fwd.get_next_point_on_chain(chosen_chain)
			guard_fwd += 1

		result.clear()
		return result

	var cur_back: TraversalJumpPoint = start_point
	var guard_back: int = 0

	while cur_back != null and guard_back < 256:
		result.append(cur_back)
		if cur_back == target_point:
			return result
		cur_back = cur_back.get_prev_point_on_chain(chosen_chain)
		guard_back += 1
...
func _try_click_jump_points(click_pos: Vector2) -> bool:
	...
	# On a ground route: keep route -> entry behavior.
	...
	# Off-route: use graph search, including bridge edges.
...
func _start_jump_chain_from_points(chain: Array[TraversalJumpPoint]) -> void:
	...
	var first_pos: Vector2 = jump_chain_positions.pop_front()
	_start_jump_to_position(first_pos)
...
func walk_to_position(world_pos: Vector2) -> void:
	...
	if best_route != null and best_route != active_route:
		var best_offset: float = best_route.get_closest_offset_to_global(world_pos)
		_clear_all_pending()
		_start_route_plan(
			best_route,
			AfterRoutePlanAction.WALK_TO_OFFSET,
...
func debug_hotspot_status(world_pos: Vector2, threshold: float = 20.0) -> void:
	print(
		"[PLAYER HOTSPOT DEBUG] player=", global_position,
		" target=", world_pos,
		" dist=", global_position.distance_to(world_pos),
		" threshold=", threshold
	)
...
func set_movement_enabled(enabled: bool) -> void:
	print("set_movement_enabled: ", enabled)
	can_move = enabled
...
func _block_scene_exits_for(seconds: float) -> void:
	...
	_scene_exit_block_until_msec = max(_scene_exit_block_until_msec, now + ms)
...
func is_scene_exit_blocked() -> bool:
	# Block exits while spawning, jumping, chaining, route transferring,
	# or during a short cooldown after spawn/landing.
	if not can_move:
		return true
